/*********************************************************************************************
Program: T1_ST1_3_5_7_9_11_antidep (antidepressants)

Study name: Pancreatic cancer is associated with multiple medication changes prior to clinical diagnosis

Study baseline: 1998 (NHS) and 1992 (HPFS)

Study endpoint: June 2012 (NHS) and Jan 2012 (HPFS)

Prupose: generate tables/figures for risk analyses conducted in the pooled cohorts by antidepressants
...
data NHS; 
   set NHS.nhsone; 
        ...
        if i=6 then do;
            ...
            %exclude(mqx eq 1); 
            %exclude(can7698 eq 1);  
            %exclude(antidepchngc eq .);                 
...
data HPFS; 
   set HPFS.hpfsone; 
        ...
        if i=4 then do;
            %exclude(exrec eq 1); 
            %exclude(0 lt dtdxca le irt92);
            %exclude(0 lt dtdth  lt irt92); 
            %exclude(dtdxca eq 9999);
...
/**STable 3 antidepressant**/
%table1(data=ALL, exposure=antidepi, agegroup=agec,
        ...
        file=ALL_antidepi.table1, uselbl=F,dec=1);

run;


/**STable 1 summary of study populations by antidepressant**/
...
%pre_pm (
    ...
    out=pmNHS, 
    case=dt_pancreas,
    var=antidepchngc antidepchngcc,
    timevar=interval, 
...
%pm (data=pmNHS, 
     case=dt_pancreas, 
     exposure=antidepchngc antidepchngcc,
     ref=1 1);
...
%pre_pm (
    ...
    out=pmHPFS, 
    timevar=interval, 
    irt=irt92 irt94 irt96 irt98 irt00 irt02 irt04 irt06 irt08 irt10, 
    cutoff=cutoff, 
    dtdx=dtdxca, 
    dtdth=dtdth, 
    case=dt_pancreas,
...
%pm(data=pmALL, case=dt_pancreas, exposure=antidepchngc antidepchngcc, ref=1 1); 

run;


/**Table 1 & STable 5_7_9_11 antidepressant**/
%mphreg9(
         data     =NHS,
         event    =dt_pancreas,
         id       =id,
         ...
         labels   =T,
        outdat    =allpacNHS,
        model1    =&antidepchngc_ ,
        model2    =&antidepchngcc_ ,
        model3    =&antidepchngc_                race  bmicum  actcum pckyr alcocum mvt db  antidep1,
        model4    =&antidepchngcc_               race  bmicum  actcum pckyr alcocum mvt db  /*antidep1*/,
        model5    =&antidepchngc_                race  &wtlosspt505c_  actcum pckyr alcocum mvt db  antidep1,
        model6    =&antidepchngcc_               race  &wtlosspt505c_  actcum pckyr alcocum mvt db  /*antidep1*/
);
run;
...
%mphreg9(
         ...
         event    =dt_pancreas,
         id       =id,
         qret     =irt92 irt94 irt96 irt98 irt00 irt02 irt04 irt06 irt08 irt10,
         cutoff   =cutoff,
         dtdth    =dtdth,
         dtdx     =dtdxca,
         timevar  =t92 t94 t96 t98 t00 t02 t04 t06 t08 t10,
         tvar     =interval,
         agevar   =agemo,
         strata   =agemo interval,
         labels   =T,
