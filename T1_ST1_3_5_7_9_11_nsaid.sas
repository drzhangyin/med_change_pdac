/*********************************************************************************************
Program: T1_ST1_3_5_7_9_11_nsaid (NSAIDs)

Study name: Pancreatic cancer is associated with multiple medication changes prior to clinical diagnosis

Study baseline: 1992 (NHS) and 1988 (HPFS)

Study endpoint: June 2012 (NHS) and Jan 2012 (HPFS)

Prupose: generate tables/figures for risk analyses conducted in the pooled cohorts by NSAIDs 
...
data NHS; 
   set NHS.nhsone; 
        ...
        if i=3 then do;
            ...
            %exclude(mqx eq 1); 
            %exclude(can7692 eq 1);  
            %exclude(anynchngc eq .);                 
...
/**STable 3 nsaid**/
%table1(data=ALL, exposure=anyni, agegroup=agec,
        ...
        file=ALL_anyni.table1, uselbl=F,dec=1);

run;


/**STable 1 summary of study populations by nsaid**/
...
%pm(data=pmALL, case=dt_pancreas, exposure=anynchngc anynchngcc, ref=1 1); 

run;


/**Table 1 & STable 5_7_9_11 nsaid**/
%mphreg9(
         data     =NHS,
         event    =dt_pancreas,
         id       =id,
        ...
        model1    =&anynchngc_ ,
        model2    =&anynchngcc_ ,
        model3    =&anynchngc_                race  bmicum  actcum pckyr alcocum mvt db  anyn1,
        model4    =&anynchngcc_               race  bmicum  actcum pckyr alcocum mvt db  /*anyn1*/,
        model5    =&anynchngc_                race  &wtlosspt505c_  actcum pckyr alcocum mvt db  anyn1,
        model6    =&anynchngcc_               race  &wtlosspt505c_  actcum pckyr alcocum mvt db  /*anyn1*/
);
run;
