/*********************************************************************************************
Program: F3_ST13_antico (anticoagulant medications)
...
Prupose: generate tables/figures for GI cancer risk analyses conducted in the pooled cohorts by anticoagulant medications to assess specificity
...
data NHS; 
   set NHS.nhsone; 
        ...
        if i=5 then do;
            %exclude(exrec eq 1);
            %exclude(0 lt dtdx_digestive le irt96); 
...
data HPFS; 
   set HPFS.hpfsone; 
        ...
        if i=7 then do;
            %exclude(exrec eq 1); 
            %exclude(0 lt dtdx_digestive le irt98);
...
%pm(data=pmALL, case=dt_digestive, exposure=coumachngc coumachngcc, ref=1 1); 

run;


/**Figure 3 & STable 13 anticoagulant medications**/
%mphreg9(
         data     =NHS,
         event    =dt_digestive,
         id       =id,
